...
border_points["callout_text"] = border_points.apply(
    lambda row: (
        ...
        f"Current     {tooltip_mmcf(row['current'], absolute=True):>7}  "
        f"{row['direction'].upper()}\n"
        f"Prorated    {tooltip_mmcf(row['prorated'], absolute=True):>7}\n"
        f"Next nom.   {tooltip_mmcf(row['nominated'], absolute=True):>7}\n"
        f"14d / 30d   {tooltip_mmcf(row['avg14'], absolute=True):>7} / "
        f"{tooltip_mmcf(row['avg30'], absolute=True)}"
    ),
    axis=1,
)
...
map_layers.append(
    pdk.Layer(
        ...
        line_height=1.32,
        background=True,
        get_background_color=[22, 28, 38, 245],
        get_border_color="status_colour",
        get_border_width=3,
        background_padding=[11, 9, 11, 9],
...
with map_col:
    ...
    st.caption(
        "Callouts stay fixed at a readable screen size as the map zooms. Border "
