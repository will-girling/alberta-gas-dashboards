...
border_points["callout_text"] = border_points.apply(
    lambda row: (
        f"{row['display_name'].upper()}\n"
        f"{tooltip_mmcf(row['current'], absolute=True)} MMcf/d "
        f"{row['direction'].lower()}\n"
        f"Prorated          {tooltip_mmcf(row['prorated'], absolute=True)}\n"
        f"Next-day nom.     {tooltip_mmcf(row['nominated'], absolute=True)}\n"
        f"14-day avg        {tooltip_mmcf(row['avg14'], absolute=True)}\n"
        f"30-day avg        {tooltip_mmcf(row['avg30'], absolute=True)}"
    ),
    axis=1,
)

# Neutral facility dots; the card border carries the relative-flow status colour.
map_layers.append(
    pdk.Layer(
        "ScatterplotLayer",
        border_points,
        ...
        get_radius="marker_size * 650",
        radius_min_pixels=3,
        radius_max_pixels=10,
        get_fill_color=[245, 247, 250, 235],
...
# Leader lines from each true marker location to its fixed card.
...
# Approximate card rectangles in geographic coordinates.
card_width_lon = 2.15
card_height_lat = 0.82
...
map_layers.append(
    pdk.Layer(
        "PolygonLayer",
        callout_polygons,
        id="interconnect-callout-backgrounds",
        get_polygon="polygon",
        filled=True,
        stroked=True,
        get_fill_color=[22, 28, 38, 238],
...
# Geographic text anchors: these move/scale with the map rather than relying
# on fixed pixel offsets.
border_points["callout_title_lat"] = border_points["callout_lat"] + 0.22
border_points["callout_body_lat"] = border_points["callout_lat"] - 0.07
...
map_layers.append(
    pdk.Layer(
        "TextLayer",
        border_points,
        id="interconnect-callout-titles",
        get_position="[callout_lon, callout_title_lat]",
...
map_layers.append(
    pdk.Layer(
        "TextLayer",
        border_points,
        id="interconnect-callout-bodies",
        get_position="[callout_lon, callout_body_lat]",
...
with map_col:
    ...
    st.caption(
        "Interconnect card borders compare absolute current flow with both the "
        "14-day and 30-day averages: green = materially above both, red = below "
        "both, amber = mixed, grey = near recent levels. Hover over pipelines for "
        f"AER segment attributes. {line_mode_text}"
    )
