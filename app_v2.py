...
def empty_time_series():
    return pd.Series(dtype='float64', index=pd.DatetimeIndex([], name='GasDay'))

def series_from(df, name, value_col, label_col):
    if not name:
        return empty_time_series()
    x = df.loc[df[label_col] == name, ['GasDay', value_col]].copy()
    x['GasDay'] = pd.to_datetime(x['GasDay'], errors='coerce').dt.normalize()
    x[value_col] = pd.to_numeric(x[value_col], errors='coerce')
    x = x.dropna(subset=['GasDay']).drop_duplicates('GasDay', keep='last')
    if x.empty:
        return empty_time_series()
    out = x.set_index('GasDay')[value_col].sort_index()
...
def snap(s,d):
    if s.empty:
        return {'current':math.nan,'previous':math.nan,'change':math.nan,'avg14':math.nan,'avg30':math.nan,'vs14':math.nan,'vs30':math.nan}
    if not isinstance(s.index, pd.DatetimeIndex):
        s = s.copy()
        s.index = pd.to_datetime(s.index, errors='coerce')
        s = s.loc[~s.index.isna()].sort_index()
    cur=value_at(s,d); prev=prev_value(s,d); a14=s.loc[:d].tail(14).mean(); a30=s.loc[:d].tail(30).mean()
...
field=opsnaps.get('Field Receipts',snap(empty_time_series(),selected)); lp=opsnaps.get('Linepack',snap(empty_time_series(),selected)); lpt=opsnaps.get('Linepack Target',snap(empty_time_series(),selected))
