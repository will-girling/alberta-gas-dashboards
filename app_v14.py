...
border_points["callout_text"] = border_points.apply(
    lambda row: (
        ...
        f"────────────────────\n"
        f"{'Current':<11}{tooltip_mmcf(row['current'], absolute=True):>7}  "
        f"{row['direction'].upper()}\n"
        f"{'Prorated':<11}{tooltip_mmcf(row['prorated'], absolute=True):>7}\n"
        f"{'Next nom.':<11}{tooltip_mmcf(row['nominated'], absolute=True):>7}\n"
        f"{'14d avg':<11}{tooltip_mmcf(row['avg14'], absolute=True):>7}\n"
        f"{'30d avg':<11}{tooltip_mmcf(row['avg30'], absolute=True):>7}"
    ),
    axis=1,
)

# Neutral geographic dots. Relative-flow status is carried by each callout border.
map_layers.append(
    pdk.Layer(
        "ScatterplotLayer",
        border_points,
...
# Leader lines from the real point to the fixed callout anchor.
...
# The callout is now one screen-space TextLayer object. The background is
# generated by deck.gl from the rendered text, so the box and font remain
# proportional and readable regardless of map zoom.
map_layers.append(
    pdk.Layer(
        "TextLayer",
        border_points,
        id="interconnect-callouts",
        get_position="[callout_lon, callout_lat]",
        get_text="callout_text",
        get_size=19,
        size_units='"pixels"',
        size_min_pixels=19,
        size_max_pixels=19,
        ...
        line_height=1.34,
        background=True,
        get_background_color=[22, 28, 38, 248],
        get_border_color="status_colour",
        get_border_width=3,
        background_padding=[9, 8, 9, 8],
...
with map_col:
    ...
    st.caption(
        "Interconnect callouts use fixed screen sizing for readability. Border colour "
        "compares absolute current flow with the 14-day and 30-day averages: green = "
        "materially above both, red = below both, amber = mixed, grey = near recent "
        f"levels. Hover over pipelines for AER segment attributes. {line_mode_text}"
    )
