...
# Preserve the NGTL sign convention in displayed flow values:
# positive = into NGTL, negative = out of NGTL.
border_points["current_signed_mmcf"] = border_points["current"]
border_points["prorated_signed_mmcf"] = border_points["prorated"]
border_points["nominated_signed_mmcf"] = border_points["nominated"]
border_points["avg14_signed_mmcf"] = border_points["avg14"]
border_points["avg30_signed_mmcf"] = border_points["avg30"]

# Relative comparisons use flow magnitude so "more outbound than usual" is
# treated as above-average activity rather than as a negative outcome.
...
border_points["tooltip_line1"] = border_points.apply(
    lambda row: (
        f"{row['current_signed_mmcf']:+,.0f} MMcf/d · "
        f"{row['direction']} · click for detail"
        if not pd.isna(row["current_signed_mmcf"])
        else "No current value · click for detail"
    ),
    axis=1,
)
...
map_layers.append(
    pdk.Layer(
        ...
        get_radius="marker_size * 1050",
        radius_min_pixels=8,
        radius_max_pixels=17,
        get_fill_color=[151, 107, 255, 245],
        get_line_color=[248, 248, 252, 255],
...
deck = pdk.Deck(
    ...
    tooltip={
        ...
        <div style="font-size:16px; font-weight:700;">
            {tooltip_title}{hover_title}
        </div>
        <div style="margin-top:4px; color:#b8c0cc;">
            {tooltip_line1}
        </div>
        """,
        "style": {
...
with system_col:
    ...
    if selected_interconnect:
        ...
        if not selected_border_rows.empty:
            ...
            with header_col:
                st.markdown(
                    f'<div class="section-label">{selected_border["display_name"]}</div>',
                    unsafe_allow_html=True,
                )
            with reset_col:
                if st.button("AB", help="Return to Alberta system metrics"):
                    st.session_state["selected_interconnect"] = None
                    st.rerun()
            ...
            st.markdown(
                ...
                ">
                    <div style="font-size:1.25rem;font-weight:760;">
                        {selected_border["current_signed_mmcf"]:+,.0f} MMcf/d
                    </div>
                    <div style="font-size:0.8rem;color:#9da7b4;margin-top:0.1rem;">
            ...
            detail_rows = pd.DataFrame(
                {
                    ...
                    "MMcf/d": [
                        selected_border["current_signed_mmcf"],
                        selected_border["prorated_signed_mmcf"],
                        selected_border["nominated_signed_mmcf"],
                        selected_border["avg14_signed_mmcf"],
                        selected_border["avg30_signed_mmcf"],
            ...
            def colour_relative_rows(row):
                styles = [""] * len(row)
                if row.name in {"vs 14-day magnitude", "vs 30-day magnitude"}:
                    value = row.iloc[0]
                    if pd.isna(value):
                        styles[0] = "color: #969faf;"
                    elif value > 0:
                        styles[0] = "color: #46be78; font-weight: 700;"
                    elif value < 0:
                        styles[0] = "color: #e65c5c; font-weight: 700;"
                    else:
                        styles[0] = "color: #969faf;"
                return styles
            ...
            st.caption(
                "Signed rows preserve the NGTL convention: positive = into NGTL, "
                "negative = out of NGTL. Relative rows compare absolute flow magnitude "
                "with the 14-day and 30-day averages; green = above average magnitude, "
