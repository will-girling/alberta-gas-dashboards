...
def iter_pipeline_segments(geojson: dict | None):
    if not geojson:
        return
    ...
    for feature in geojson.get("features", []):
        ...
        if geom_type == "LineString":
            yield properties, coordinates

        elif geom_type == "MultiLineString":
            for line in coordinates:
                yield properties, line
...
if show_ngtl_pipelines and ngtl_pipeline_geojson:
    first_pipeline_trace = True

    for properties, coordinates in iter_pipeline_segments(
        ...
        licence = properties.get("LICENCE_NO", "—")
        line_no = properties.get("LINE_NO", "—")
        segment_id = properties.get("PLLICSEGID", "—")
        diameter = properties.get("OUT_DIAMET", "—")
        substance = properties.get("SUBSTANCE1", "—")
        status = properties.get("SEG_STATUS", "—")
        length_km = properties.get("SEG_LENGTH", "—")
        bidirectional = properties.get("BIDIRE_IND", "—")
        company = properties.get("COMP_NAME", "NOVA Gas Transmission Ltd.")
        ...
        hover_text = (
            ...
            f"Diameter: {diameter} mm<br>"
            f"Substance: {substance}<br>"
            f"Status: {status}<br>"
            f"Length: {length_km} km<br>"
        ...
        fig.add_trace(
            go.Scattermap(
                ...
                name="NGTL pipelines",
                legendgroup="ngtl_pipelines",
                showlegend=first_pipeline_trace,
            )
        )

        first_pipeline_trace = False
...
with map_col:
    ...
    st.caption(
        "Border-marker positions remain approximate. Hover over an NGTL pipeline "
        "segment to view its licence, line, diameter, substance and status."
    )
