...
border_points = pd.DataFrame(
    [
        ...
        {
            "label": "Gordondale",
            "display_name": "Gordondale Interconnect",
            "lat": 55.80,
            "lon": -119.98,
            "label_side": "below",
        },
        {
            ...
            "display_name": "Groundbirch East Interconnect",
            "lat": 55.78,
            "lon": -120.62,
            "label_side": "above",
        },
...
def tooltip_mmcf(value: float, absolute: bool = False) -> str:
    ...
    return f"{converted:,.0f} MMcf/d"


border_points["flow_color"] = border_points["current"].map(
    lambda value: (
        "#55d68b"
...
border_points["tooltip_line1"] = border_points.apply(
    lambda row: (
        f"Current extrapolated: "
        f"{tooltip_mmcf(row['current'], absolute=True)} "
        f"({row['direction'].lower()})"
    ),
    axis=1,
)
border_points["tooltip_line2"] = border_points.apply(
    lambda row: f"Prorated: {tooltip_mmcf(row['prorated'], absolute=True)}",
    axis=1,
)
border_points["tooltip_line3"] = border_points.apply(
    lambda row: (
        f"Next-day nominated: "
        f"{tooltip_mmcf(row['nominated'], absolute=True)}"
    ),
    axis=1,
)
border_points["tooltip_line4"] = border_points.apply(
    lambda row: (
        f"Day-over-day change: "
        f"{tooltip_mmcf(row['change'], absolute=False)}"
    ),
    axis=1,
)
border_points["tooltip_line5"] = border_points.apply(
    lambda row: f"14-day average: {tooltip_mmcf(row['avg14'], absolute=True)}",
    axis=1,
)
border_points["tooltip_line6"] = border_points.apply(
    lambda row: f"30-day average: {tooltip_mmcf(row['avg30'], absolute=True)}",
    axis=1,
)
...
deck = pdk.Deck(
    ...
    tooltip={
        "html": """
        <div style="font-size:17px; font-weight:700; margin-bottom:9px;">
            {tooltip_title}
        </div>
        <div style="line-height:1.6; color:{flow_color}; font-weight:600;">
            {tooltip_line1}<br/>
            {tooltip_line2}<br/>
            {tooltip_line3}<br/>
            {tooltip_line4}<br/>
