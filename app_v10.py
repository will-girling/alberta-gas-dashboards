...
def selected_flow_value(label: str, column: str) -> float:
    ...
    values = selected_flow_rows.loc[
        selected_flow_rows["Item"].astype(str) == str(source_name),
        column,
    ]
...
def tooltip_bcf(value: float, absolute: bool = False) -> str:
    if pd.isna(value):
        return "—"
    converted = mmcf_to_bcf(value)
    if absolute:
        converted = abs(converted)
    return f"{converted:.2f} Bcf/d"
...
border_points["tooltip_line1"] = border_points.apply(
    lambda row: (
        ...
        f"{tooltip_bcf(row['current'], absolute=True)} "
        f"({row['direction'].lower()})"
    ),
    axis=1,
)
border_points["tooltip_line2"] = border_points.apply(
    lambda row: f"Prorated: {tooltip_bcf(row['prorated'], absolute=True)}",
    axis=1,
)
border_points["tooltip_line3"] = border_points.apply(
    lambda row: (
        f"Next-day nominated: "
        f"{tooltip_bcf(row['nominated'], absolute=True)}"
    ),
    axis=1,
)
border_points["tooltip_line4"] = border_points.apply(
    lambda row: (
        ...
        f"{tooltip_bcf(row['change'], absolute=False)}"
    ),
    axis=1,
)
border_points["tooltip_line5"] = border_points.apply(
    lambda row: f"14-day average: {tooltip_bcf(row['avg14'], absolute=True)}",
    axis=1,
)
border_points["tooltip_line6"] = border_points.apply(
    lambda row: f"30-day average: {tooltip_bcf(row['avg30'], absolute=True)}",
    axis=1,
)
