...
border_points["current_abs_mmcf"] = border_points["current"].abs()
border_points["prorated_abs_mmcf"] = border_points["prorated"].abs()
border_points["nominated_abs_mmcf"] = border_points["nominated"].abs()
border_points["avg14_abs_mmcf"] = border_points["avg14"].abs()
border_points["avg30_abs_mmcf"] = border_points["avg30"].abs()

border_points["vs14_abs_mmcf"] = (
    border_points["current_abs_mmcf"] - border_points["avg14_abs_mmcf"]
)
border_points["vs30_abs_mmcf"] = (
    border_points["current_abs_mmcf"] - border_points["avg30_abs_mmcf"]
)

def fmt_hover(value: float) -> str:
    if pd.isna(value):
        return "—"
    return f"{value:,.0f}"

def fmt_hover_signed(value: float) -> str:
    if pd.isna(value):
        return "—"
    return f"{value:+,.0f}"

border_points["hover_title"] = border_points["display_name"]
border_points["hover_current"] = border_points["current_abs_mmcf"].map(fmt_hover)
border_points["hover_prorated"] = border_points["prorated_abs_mmcf"].map(fmt_hover)
border_points["hover_nominated"] = border_points["nominated_abs_mmcf"].map(fmt_hover)
border_points["hover_avg14"] = border_points["avg14_abs_mmcf"].map(fmt_hover)
border_points["hover_avg30"] = border_points["avg30_abs_mmcf"].map(fmt_hover)
border_points["hover_vs14"] = border_points["vs14_abs_mmcf"].map(fmt_hover_signed)
border_points["hover_vs30"] = border_points["vs30_abs_mmcf"].map(fmt_hover_signed)
border_points["hover_direction"] = border_points["direction"].str.title()
border_points["tooltip_title"] = border_points["display_name"]
border_points["tooltip_line1"] = "Click marker for detail"
border_points["tooltip_line2"] = ""
border_points["tooltip_line3"] = ""
border_points["tooltip_line4"] = ""
border_points["tooltip_line5"] = ""
border_points["tooltip_line6"] = ""

border_points["hover_status"] = border_points["status"].map(
    {
        "above": "Above recent averages",
        "below": "Below recent averages",
        "mixed": "Mixed vs recent averages",
        "neutral": "Near recent averages",
    }
)

# Small interconnect points with hover. Colour reflects relative position
# versus 14-day and 30-day absolute flow averages.
map_layers.append(
    pdk.Layer(
        "ScatterplotLayer",
        border_points,
        ...
        get_radius="marker_size * 700",
        radius_min_pixels=5,
        radius_max_pixels=11,
        get_fill_color="status_colour",
        get_line_color=[245, 247, 250, 255],
        line_width_min_pixels=1.2,
        stroked=True,
    )
)
    ...
    tooltip={
        ...
        <div style="min-width:340px;">
          <div style="font-size:18px; font-weight:750; margin-bottom:10px;">
            {hover_title}{tooltip_title}
          </div>

          <table style="border-collapse:collapse; width:100%; line-height:1.55;">
            <tr><td style="padding-right:18px; color:#aeb8c5;">Current</td><td>{hover_current} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Prorated</td><td>{hover_prorated} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Next-day nominated</td><td>{hover_nominated} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">14-day average</td><td>{hover_avg14} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">30-day average</td><td>{hover_avg30} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Direction</td><td>{hover_direction}</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">vs 14d</td><td>{hover_vs14} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">vs 30d</td><td>{hover_vs30} MMcf/d</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Status</td><td>{hover_status}</td></tr>

            <tr><td colspan="2" style="height:8px;"></td></tr>

            <tr><td style="padding-right:18px; color:#aeb8c5;">Licence / line</td><td>{tooltip_line1}</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Diameter</td><td>{tooltip_line2}</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Substance</td><td>{tooltip_line3}</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Status</td><td>{tooltip_line4}</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Length</td><td>{tooltip_line5}</td></tr>
            <tr><td style="padding-right:18px; color:#aeb8c5;">Segment</td><td>{tooltip_line6}</td></tr>
          </table>
        </div>
        """,
        "style": {
...
with map_col:
    ...
    if "selected_interconnect" not in st.session_state:
        st.session_state["selected_interconnect"] = "Empress"
    ...
    try:
        ...
        if selected_border_objects:
            selected_label = selected_border_objects[0].get("label")
            if selected_label:
...
with system_col:
    ...
    st.markdown("#### Interconnect detail")

    selected_interconnect = st.session_state.get(
        "selected_interconnect",
        "Empress",
    )

    selected_border_rows = border_points.loc[
        border_points["label"] == selected_interconnect
    ]

    if not selected_border_rows.empty:
        selected_border = selected_border_rows.iloc[0]
        ...
        status_label = {
            ...
            "mixed": "Mixed vs recent averages",
            "neutral": "Near recent averages",
        }.get(selected_border["status"], "No status")
        ...
        status_css = {
            ...
            "mixed": "#ebb14a",
            "neutral": "#969faf",
        }.get(selected_border["status"], "#969faf")

        st.markdown(
            f"""
            <div style="
                border:1px solid rgba(255,255,255,0.11);
                border-left:5px solid {status_css};
                background:rgba(25,31,41,0.96);
                border-radius:10px;
                padding:0.8rem 0.95rem;
                margin:0.2rem 0 0.65rem 0;
            ">
                <div style="font-size:1.05rem;font-weight:750;">
                    {selected_border["display_name"]}
                </div>
                <div style="font-size:0.78rem;color:#9da7b4;margin-top:0.15rem;">
                    {selected_border["direction"]} · {status_label}
                </div>
            </div>
            """,
            unsafe_allow_html=True,
        ...
        detail_rows = pd.DataFrame(
            {
                ...
                "MMcf/d": [
                    selected_border["current_abs_mmcf"],
                    selected_border["prorated_abs_mmcf"],
                    selected_border["nominated_abs_mmcf"],
                    selected_border["avg14_abs_mmcf"],
                    selected_border["avg30_abs_mmcf"],
                    selected_border["vs14_abs_mmcf"],
                    selected_border["vs30_abs_mmcf"],
                ],
            }
        ).set_index("Metric")

        st.dataframe(
            detail_rows.style.format(
                {"MMcf/d": "{:+,.0f}"},
                na_rep="—",
            ),
            use_container_width=True,
        ...
        st.caption(
            "Current, prorated, nominated and rolling-average rows use absolute "
            "flow magnitude. The vs-average rows retain their sign."
        )
    else:
        st.info("Click an interconnect marker on the map to view its details.")
