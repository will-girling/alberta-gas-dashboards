...
border_points = pd.DataFrame(
    [
        # Northern BC / northwest Alberta interconnect area
        {"label": "Gordondale", "lat": 55.82, "lon": -120.12, "label_side": "above"},
        {"label": "Groundbirch East", "lat": 55.78, "lon": -120.62, "label_side": "above"},
        {"label": "Willow Valley", "lat": 55.66, "lon": -120.55, "label_side": "below"},

        # Southern Alberta–BC export corridor near Crowsnest Pass
        {"label": "Alberta–BC", "lat": 49.63, "lon": -114.69, "label_side": "above"},

        # Eastern export corridor into Saskatchewan
        {"label": "Empress", "lat": 50.95, "lon": -110.01, "label_side": "above"},
        {"label": "McNeill", "lat": 50.15, "lon": -110.02, "label_side": "below"},
    ]
)
...
border_points["current_label"] = border_points["current_bcf_abs"].map(
    lambda x: "—" if pd.isna(x) else f"{x:.2f} Bcf/d"
)

border_points["tooltip_text"] = border_points.apply(
    lambda row: (
        f"{row['label']}\\n"
        f"{row['current_label']}\\n"
        f"{row['direction']}"
...
map_layers.append(
    pdk.Layer(
        ...
        get_radius="marker_size * 950",
        radius_min_pixels=5,
        radius_max_pixels=16,
        get_fill_color=[255, 255, 255, 225],
...
deck = pdk.Deck(
    ...
    tooltip={
        "html": """
        <b>{COMP_NAME}</b><br/>
        Licence: {LICENCE_NO}<br/>
        Line: {LINE_NO}<br/>
        Segment ID: {PLLICSEGID}<br/>
        Diameter: {OUT_DIAMET} mm<br/>
        Substance: {SUBSTANCE1}<br/>
        Status: {SEG_STATUS}<br/>
        Length: {SEG_LENGTH} km<br/>
        Bidirectional: {BIDIRE_IND}
        """,
        "style": {
...
with map_col:
    ...
    st.caption(
        "Border-marker positions remain approximate. Hover over pipeline geometry "
        f"to view AER segment attributes. {line_mode_text}"
    )
